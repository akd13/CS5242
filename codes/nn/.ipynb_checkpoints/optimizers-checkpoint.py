    ...
    def update(self, xs, xs_grads, iteration):
        ...
        for k in list(xs.keys()):
            ...
            if self.bias_correction:
                avg_moment = self.moments[k] / (1 - self.beta_1**(iteration+1)) # handle first iteration, iteration = 0
                avg_accumulator = self.accumulators[k] / (1 - self.beta_2**(iteration+1))
                new_xs[k] = xs[k] - self.lr * avg_moment / (np.sqrt(avg_accumulator) + eps)
            else:
